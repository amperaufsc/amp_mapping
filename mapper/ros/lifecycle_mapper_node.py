...
class MapperNode(LifecycleNode):
    def __init__(self):
        ...
        self.as_driving = False
        #self.trans = None

        self.queue_size = 10
        self.max_delay = 10000000
        self.time_gap = 1
        self.node_name = 'mapper_node_lifecycle'
        self.get_logger().info('PAAAAAAAAAAAAAAUUUUUUUUUUUUUUUUUU')
    ...
    def sync_callback(self,track,odom):
            if not self.is_activated:
                return
            self.get_logger().info('CUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU')
            tempo = 0
            if tempo < self.time_gap: 
                if  self.transform_received:
                    rotate_track =self.rotate_track(track,trans)
                    ...
                    self.get_logger().info('TENHO DEPRESSAO SEVERA')
                    self.track_pub.publish(track)
                else:
                    try:
                        trans=self._tf_buffer.lookup_transform("left_camera_link","oak_left_camera_optical_frame",rclpy.time.Time())
                        self.get_logger().info(trans.child_frame_id)
    ...
    def track_to_obstacle(self, track):
        ...
        for cone in track.track:
            ...
            if cone.color == 0:
                ...
                color = 4
            obstacle = Obstacle(x,y,1,color, deviation=0.5)
